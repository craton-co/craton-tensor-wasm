...
Hardware-gated and Scaffold are deliberately *not* called "Wired": a
green default-build test does **not** prove the driver call works.
...
| GPU memory quotas (driver `cuMemPool` pin) | `tensor-wasm-mem` | Hardware-gated | `gpu-mem-pool` (strict-superset of `cudarc-backend`) | T39. `TenantMemPool` pins the cap via `cuMemPoolSetAttribute(CU_MEMPOOL_ATTR_RELEASE_THRESHOLD, …)` and routes allocations through `cuMemAllocFromPoolAsync` (`UnifiedBuffer::new_in_tenant_pool`). Requires CUDA 11.2+; the driver-reject test is `#[ignore]`. Bypass-resistant second line of defence. |
...
| Real CUDA host functions / kernel launch | `tensor-wasm-exec`, `tensor-wasm-wasi-gpu`, `tensor-wasm-snapshot`, `tensor-wasm-tenant` | Hardware-gated | `cuda` | Real `wasi:cuda` host fns, contexts, `cuLaunchKernel`, GPU snapshot restore. Typed argv lowering for scalar + pointer kernel args (W1.1). End-to-end launch tests `#[ignore]`. |
