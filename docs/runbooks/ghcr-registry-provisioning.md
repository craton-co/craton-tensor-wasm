...
- **The Dockerfile at repo root builds clean** on a dev box. The C8
  ...
  validated on the workspace at `0.3.4`; if more than one minor version
...
docker build -t ghcr.io/craton-co/tensor-wasm:0.3.4-local .
...
docker push ghcr.io/craton-co/tensor-wasm:0.3.4-local
```

After the push completes, open
`https://github.com/orgs/craton-co/packages` — the
`tensor-wasm` package should appear with the `0.3.4-local` tag. If it
...
```sh
# List versions to find the version ID for the 0.3.4-local tag.
...
`docker pull ghcr.io/craton-co/tensor-wasm:0.3.4` will not
...
```sh
docker pull ghcr.io/craton-co/tensor-wasm:0.3.4
docker run --rm ghcr.io/craton-co/tensor-wasm:0.3.4 --version
```

Expected output: `tensor-wasm 0.3.4` (or whatever version was just
...
docker run -d --name twasm-smoke -p 8080:8080 \
    ...
    ghcr.io/craton-co/tensor-wasm:0.3.4
...
`docker run --rm --entrypoint sh ghcr.io/craton-co/tensor-wasm:0.3.4 -c 'which curl'`).
...
lags the workspace version today — it is pinned at `0.3.3` even
though the workspace is at `0.3.4`. Once the registry is provisioned,
the chart's default `image.tag` resolves through `appVersion`, so the
lag becomes user-visible (`helm install` pulls the wrong image).

Bump it as part of the same PR as Step 6, or as a follow-up:

1. Edit `Chart.yaml`: `appVersion: "0.3.4"` (or whatever the latest
   released tag is).
...
docker tag ghcr.io/craton-co/tensor-wasm:0.3.4 ghcr.io/craton-co/tensor-wasm:0.3.4-deprecated
...
docker buildx imagetools create \
    ...
    ghcr.io/craton-co/tensor-wasm:0.3.4 \
    --tag ghcr.io/craton-co/tensor-wasm:0.3.4
```
