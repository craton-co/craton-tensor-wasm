...
Project Bali's GPU-resident crates — `bali-mem`, `bali-wasi-gpu`, `bali-jit`, and `bali-tenant` — all link against the CUDA driver API and CUDA runtime via the [`cust`](https://docs.rs/cust) crate. To build and test these crates locally you need a working CUDA toolkit, a matching NVIDIA driver, and a handful of environment variables wired up so `nvcc`, `ptxas`, and the `cust` build script can find the headers and stub libraries. This document walks through toolkit installation across the supported host operating systems, the driver-version matrix Project Bali validates against, the required environment variables, and a smoke test you can run before opening a PR.
...
The CUDA runtime is forward-compatible only against drivers at or above the toolkit's minimum. Project Bali CI matrix-tests against CUDA 12.0 and 12.4 stubs; 12.6 is the recommended local-dev version.
...
If `nvidia-smi` reports a driver below the row for your installed toolkit, either upgrade the driver or downgrade the toolkit — mismatches surface as `CUDA_ERROR_SYSTEM_DRIVER_MISMATCH` (error 803) at the first `cuInit` call inside `bali-mem`.

## Required environment variables

Project Bali's build scripts read four variables. Set them globally (shell profile, `setx`) rather than per-shell so `rust-analyzer` and your IDE pick them up too.
...
`CUDA_ARCH` accepts the values you'd pass to `nvcc -arch=`: `sm_80` for A100, `sm_86` for RTX 30xx (Ampere consumer), `sm_89` for L4 / L40S / RTX 40xx, and `sm_90` for H100. The S12 PTX emitter defaults to `sm_80` because that's the lowest capability that supports the async-copy and tensor-memory-accelerator intrinsics `bali-jit` lowers to.
...
ptxas --version        # PTX assembler reachable; bali-jit (S12) depends on this
```
...
A clean build of `bali-mem`, `bali-wasi-gpu`, and `bali-jit` against a real toolkit is the canonical smoke test. If `cargo` fails at the `cust-build` stage with `could not find libcuda`, re-check that `CUDA_ROOT/lib64` (or `CUDA_ROOT\lib\x64` on Windows) is on the loader path.

## Stub libraries for CI

GitHub-hosted runners have no GPU, so the Project Bali CI workflow does **not** install the real CUDA toolkit. Instead, `.github/workflows/ci.yml` drops a directory of stub `.so` files at `/usr/local/cuda/lib64/` containing only the symbols `cust` resolves at link time (`cuInit`, `cuMemAlloc`, `cuLaunchKernel`, etc.) — each as a no-op exported from a tiny C shim. This is enough to satisfy the linker so the workspace builds, but any test that actually launches a kernel is gated behind `#[cfg(feature = "gpu-runtime")]` and skipped on CI. Running real CUDA kernels in CI requires self-hosted runners with attached GPUs, which is out of scope until S22 (the perf-regression milestone).
...
---
_Updated for bali v0.1.0 (S2 of plan). Re-verify driver matrix before each release._
