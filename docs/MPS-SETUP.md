...
`bali-tenant` probes `/tmp/nvidia-mps` (the value of [`bali_tenant::MPS_CONTROL_PATH`]) at registry construction time; if you point `CUDA_MPS_PIPE_DIRECTORY` somewhere else you also need to symlink or override the probe path before deploying. To stop the daemon cleanly so it flushes per-client state, send `quit` on its control channel:
...
Inside containers, the orchestrator must pass `--cap-add SYS_NICE` (Docker) or set `securityContext.capabilities.add: ["SYS_NICE"]` (Kubernetes) on the pod running the daemon. The MPS *clients* — that is, `bali-cli` and the `bali-api` server processes — do not need this capability.

## Feature limitations

MPS is not a substitute for hardware partitioning. The most important constraints to remember when planning a deployment:

- **No on-the-fly resize.** Per-client thread-percent and memory limits are set when the client first connects. To change a tenant's share you must `quit` the daemon, edit `CUDA_MPS_ACTIVE_THREAD_PERCENTAGE` or per-client overrides, and restart — every existing client loses its context. The Bali scheduler treats MPS quotas as immutable for the lifetime of a tenant registration.
...
- **No CUDA-graphics interop.** OpenGL / DirectX interop is disabled under MPS. Bali kernels are pure compute, so this does not affect us, but be aware if you embed Bali in a larger application that wants to render with the same GPU.
- **Single-user by default.** All MPS clients must share a UID with the daemon unless you run the daemon in exclusive-process compute mode and configure per-user pipe directories. The Bali deployment template runs the API server as the `nvidia-mps` user for this reason.
