...
| Chart version | `0.1.1` |
| App version | `0.3.7` |
| Default image | `ghcr.io/craton-co/tensor-wasm:0.3.7` (host-only); `…:0.3.7-cust` / `…-cudarc` / `…-cuda-oxide` when `image.backend` is set |
| Kubernetes | `>= 1.23` |

> **Image registry is not yet provisioned.** The `ghcr.io/craton-co/*` path is
> aspirational as of v0.3.7 — operators must build + push locally until the
...
> ```sh
> docker build                                  -t my-registry/tensor-wasm:0.3.7            .
> docker build --build-arg BACKEND=cust         -t my-registry/tensor-wasm:0.3.7-cust       .
> docker build --build-arg BACKEND=cudarc       -t my-registry/tensor-wasm:0.3.7-cudarc     .
> docker build --build-arg BACKEND=cuda-oxide   -t my-registry/tensor-wasm:0.3.7-cuda-oxide .
> ```
...
**Today (v0.3.7) this toggle is a "documentation handle".** The
