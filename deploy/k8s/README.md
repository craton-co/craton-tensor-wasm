...
> `ghcr.io/craton-co/tensor-wasm:0.3.5`. The `ghcr.io/craton-co/*`
> registry is not yet provisioned (v0.3.5 era of the project). Until it
...
- [ ] **A GPU-enabled tensor-wasm image** built with the `unified-memory`
      ...
      `ghcr.io/craton-co/tensor-wasm:0.3.5` is host-only and will
