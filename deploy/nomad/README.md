...
> `ghcr.io/craton-co/tensor-wasm:0.1.0`. The raw_exec spec fetches
> `https://example.invalid/.../tensor-wasm-x86_64-linux`. Neither URL
> resolves yet (v0.1.0 era). Until the `ghcr.io/craton-co/*` registry
...
4. Un-comment the `template { ... }` stanza in the task. The default
   ...
   process supports it (v0.1.0 does not — restart is the safe choice).
...
4. **GPU-enabled image** built with the `unified-memory` / `cuda`
   feature set. The default `ghcr.io/craton-co/tensor-wasm:0.1.0` is
