...
impl fmt::Display for TensorWasmOp {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            TensorWasmOp::VecAdd { lanes } => write!(f, "vec_add[{lanes}]"),
            TensorWasmOp::VecMul { lanes } => write!(f, "vec_mul[{lanes}]"),
            TensorWasmOp::VecFma { lanes } => write!(f, "vec_fma[{lanes}]"),
            TensorWasmOp::MatMul { m, n, k } => write!(f, "matmul[{m}x{n}x{k}]"),
            TensorWasmOp::LoadUnified { lanes } => write!(f, "load_unified[{lanes}]"),
            TensorWasmOp::StoreUnified { lanes } => write!(f, "store_unified[{lanes}]"),
...
impl TensorWasmKernelBlueprint {
    ...
    /// byte BLAKE3 digest are interpreted as a little-endian u64 for the
    /// `KernelCache` key.
    pub fn fingerprint(&self) -> u64 {
        let mut hasher = blake3::Hasher::new();
        // Domain-separate so future changes to the hashed schema (e.g. a new
        // op variant) can never collide with v0 fingerprints — bump this on
        // any schema change.
        hasher.update(b"tensor-wasm-jit::ir::v1\0");
...
mod tests {
    ...
    fn fingerprint_is_deterministic() {
        let a = TensorWasmKernelBlueprint::new("k").push(TensorWasmOp::VecAdd { lanes: 4 });
        let b = TensorWasmKernelBlueprint::new("k").push(TensorWasmOp::VecAdd { lanes: 4 });
    ...
    fn fingerprint_changes_with_lanes() {
        let a = TensorWasmKernelBlueprint::new("k").push(TensorWasmOp::VecAdd { lanes: 4 });
        let b = TensorWasmKernelBlueprint::new("k").push(TensorWasmOp::VecAdd { lanes: 8 });
    ...
    fn fingerprint_is_blake3_based_and_pinned() {
        ...
        let other = TensorWasmKernelBlueprint::new("vector_add")
            .push(TensorWasmOp::LoadUnified { lanes: 4 })
            .push(TensorWasmOp::VecMul { lanes: 4 })
            .push(TensorWasmOp::StoreUnified { lanes: 4 });
    ...
    #[test]
    fn op_display() {
        assert_eq!(TensorWasmOp::VecAdd { lanes: 4 }.to_string(), "vec_add[4]");
        assert_eq!(
            TensorWasmOp::MatMul {
                m: 16,
                n: 16,
