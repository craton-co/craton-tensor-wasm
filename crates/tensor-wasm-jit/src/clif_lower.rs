...
/// | Source                                       | Target                          |
/// |----------------------------------------------|---------------------------------|
/// | `Op::V128Add`                                | `TensorWasmOp::VecAdd { lanes: 4 }`   |
/// | `Op::V128Mul`                                | `TensorWasmOp::VecMul { lanes: 4 }`   |
/// | `Op::V128Fma`                                | `TensorWasmOp::VecFma { lanes: 4 }`   |
/// | `Op::Load`                                   | `TensorWasmOp::LoadUnified { 4 }`     |
/// | `Op::Store`                                  | `TensorWasmOp::StoreUnified { 4 }`    |
...
pub fn lower_block(block: &BlockIR) -> Result<TensorWasmKernelBlueprint, LowerError> {
    ...
    for op in &block.ops {
        let lo = match op {
            Op::V128Add => TensorWasmOp::VecAdd {
                lanes: DEFAULT_LANES,
            },
            Op::V128Mul => TensorWasmOp::VecMul {
                lanes: DEFAULT_LANES,
            },
            Op::V128Fma => TensorWasmOp::VecFma {
                lanes: DEFAULT_LANES,
            },
...
mod tests {
    ...
    fn lower_simple_vector_add() {
        ...
        assert!(matches!(bp.ops[2], TensorWasmOp::VecAdd { lanes: 4 }));
