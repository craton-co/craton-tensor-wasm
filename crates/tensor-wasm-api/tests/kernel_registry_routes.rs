...
/// [`TEST_KEY`]. Dev-mode auth so the bearer-auth layer does not gate
/// the test calls.
...
fn dev_router(state: Arc<AppState>) -> axum::Router {
    build_router_with_config(state, AuthConfig::default(), TenantConfig::default())
}
