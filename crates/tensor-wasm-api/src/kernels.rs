...
use tensor_wasm_jit::registry::KernelRegistry as _;
...
pub async fn publish_kernel(
    ...
    registry
        ...
        .map_err(|e| match e {
            ...
            //
            // M-2: the inner `ArtifactError` Display (carried verbatim in
            // `RegistryError::Storage`) can embed host filesystem paths or
            // backend internals — leaking it to the client is an
            // information-disclosure bug. Log the full chain server-side
            // (matching the `sanitised_exec_error_message` /
            // `ExecError::Wasmtime` pattern in `routes.rs`) and return a
            // stable, content-light message instead of `e.to_string()`.
            tensor_wasm_jit::registry::RegistryError::Storage(_) => {
                tracing::error!(
                    target: "tensor_wasm_api::kernels",
                    error = %e,
                    "kernel publish failed: artifact-store error mapped to 503",
                );
                ApiError::service_unavailable(
                    "kernel_registry_storage_error",
                    "kernel registry storage error",
                )
            }
            ...
            // that resolves this.
            //
            // M-2: the inner bincode Display is internal serialization
            // detail of the on-disk blob format and is of no use to the
            // client. Log it server-side and return a stable, content-light
            // 500 message rather than echoing the codec error verbatim.
            tensor_wasm_jit::registry::RegistryError::Codec(_) => {
                tracing::error!(
                    target: "tensor_wasm_api::kernels",
                    error = %e,
                    "kernel publish failed: manifest codec error mapped to 500",
                );
                ApiError::internal("kernel registry internal error")
            }
            ...
            //
            // M-2: because this arm absorbs *unknown* future variants, we
            // cannot vouch for what their `Display` embeds (it may carry
            // host/internal detail). Log the full `e` server-side and
            // return a stable, content-light message; this preserves the
            // 400 `invalid_request` envelope without echoing arbitrary
            // future Display strings to clients.
            _ => {
                tracing::warn!(
                    target: "tensor_wasm_api::kernels",
                    error = %e,
                    "kernel publish failed: unmapped registry error mapped to 400",
                );
                ApiError::bad_request("invalid_request", "invalid kernel publish request")
            }
        })?;
    Ok((
        StatusCode::CREATED,
