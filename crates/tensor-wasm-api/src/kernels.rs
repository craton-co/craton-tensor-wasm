...
//!
//! All three routes are bearer-authed; only operators with a
//! `kernel-publish` scope can call `POST` (the v0.4 scope-table wiring
//! is a follow-up — until it lands every authenticated caller can
//! publish, gated by the server-side HMAC verification).
//!
...
/// The handler:
///
/// 1. Pulls the registry out of [`AppState`]; returns `503` when unset.
/// 2. Hands the (manifest, ptx_text) pair to
...
/// 3. Maps each [`RegistryError`](tensor_wasm_jit::registry::RegistryError)
...
///
/// The `_tenant` extension is accepted but not yet consumed: v0.4 will
/// thread the publisher's tenant into a per-tenant key-scope table; the
/// scaffold here keeps the extractor wired so a follow-up does not
/// re-shape the handler signature.
