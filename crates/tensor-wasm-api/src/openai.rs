...
impl OpenAiError {
    ...
    /// [`ApiError::forbidden`](crate::routes::ApiError::forbidden) with
...
impl IntoResponse for OpenAiError {
    fn into_response(self) -> Response {
        ...
        // mirror that wire shape here, and additionally map our
        // `tenant_scope_denied` code to `403` for the per-resource owner
        // check (api S-IDOR).
...
///    A hit owned by a different tenant → `403 tenant_scope_denied`
///    (per-resource owner check, api S-IDOR).
...
pub async fn completions_handler(
    ...
    // Fail closed if the AuthContext extension is absent (router wired
    // without `bearer_auth`): `require_authorize` surfaces `500 internal`
    // rather than silently skipping the per-tenant scope check.
...
async fn run_translated(
    ...
    let wasm_bytes = match state.functions.get(&translated.function_id) {
        Some(entry) => {
            // Per-resource owner check (api S-IDOR): the model→function map
            // is process-global, so a model alias could resolve to a record
            // owned by a different tenant. The native invoke handlers enforce
            // `FunctionRecord::tenant_id == tenant` before touching the bytes
            // (see routes.rs); mirror that here so a wildcard-scoped caller
            // from tenant B cannot drive tenant A's function. Capture
            // `tenant_id` from the same `entry` ref before it is dropped.
