...
impl BusyPollBackend {
    fn new() -> Self {
        Self { back_pressure: Arc::new(BackPressure::with_cap(1)) }
    }
}
...
impl DispatchFutureBackend for BusyPollBackend {
    ...
    fn name(&self) -> &'static str { "busy-poll" }
}
...
impl DispatchFutureBackend for CudaAsyncBackend {
    ...
    fn name(&self) -> &'static str { "cuda-async" }
}
...
struct TailStats {
    backend: String,
    samples: usize,
    p50_ns: u128, p95_ns: u128, p99_ns: u128, p99_9_ns: u128, max_ns: u128,
}
...
pub fn run() {
    ...
    run_backend(&rt, &BusyPollBackend::new(), &mut json_lines,
        "unexpected stub return -- check trait contract");
    run_backend(&rt, &CudaAsyncBackend, &mut json_lines, CUDA_ASYNC_NOT_YET_WIRED);
    ...
    if let Some(dir) = workspace_bench_results_dir() {
        ...
        if let Err(e) = fs::write(&path, render_file(&json_lines)) {
            eprintln!("DISPATCH_BACKEND warn: could not write {}: {}", path.display(), e);
        }
    }
}
...
mod tests {
    ...
    fn cuda_async_returns_sentinel() {
        ...
        let err = rt.block_on(CudaAsyncBackend.dispatch_once())
    ...
    fn bench_one_returns_some_for_busy_poll_and_none_for_stub() {
        ...
        assert!(bench_one(&rt, &CudaAsyncBackend, 64).is_none(),
