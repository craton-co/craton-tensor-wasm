...
impl TensorWasmEngine {
    ...
    pub fn with_config(cfg: EngineConfig) -> Result<Self, wasmtime::Error> {
        ...
        // Defense-in-depth (L-4): the remaining audited-out proposals would be
        // pinned `false` here too, but on wasmtime 45 their `Config` setters do
        // not exist in this build — they are `#[cfg]`-gated behind crate
        // features we deliberately do NOT enable, so the proposals are fully
        // compiled out of the engine and config cannot turn them on:
        //   - `wasm_reference_types(false)`   -> gated `feature = "gc"`     (off)
        //   - `wasm_function_references(false)`-> gated `feature = "gc"`     (off)
        //   - `wasm_gc(false)`                -> gated `feature = "gc"`     (off)
        //   - `wasm_threads(false)`           -> gated `feature = "threads"`(off)
        // If a future change pulls in the `gc` and/or `threads` wasmtime
        // features, un-comment the corresponding setters below so the deny-list
        // stays explicit rather than relying on the proposals being absent:
        //   wt_cfg.wasm_reference_types(false);
