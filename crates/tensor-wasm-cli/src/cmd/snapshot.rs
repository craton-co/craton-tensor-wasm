...
/// truncated key.
///
/// Key-file permission handling is platform-dependent (sec LOW / L-8):
///
/// * **Unix:** a group/other-readable key file (mode `& 0o077 != 0`) is
///   warned about by default; with `TENSOR_WASM_REQUIRE_KEY_PERMS=1` it is a
///   hard error.
/// * **Windows:** POSIX permission bits don't exist and NTFS-ACL parsing is
///   out of scope, so no permission check is performed. By default this is
///   surfaced as a one-line informational warning; with
///   `TENSOR_WASM_REQUIRE_KEY_PERMS=1` we fail closed and refuse, since we
///   cannot honor the strict guarantee. Operators must secure the key file via
///   NTFS ACLs.
pub(crate) fn load_hmac_key(path: &Path) -> Result<zeroize::Zeroizing<[u8; 32]>> {
    ...
    //
    // sec LOW (L-8): the permission check above is Unix-only. On Windows we
    // cannot cheaply express the POSIX `0o077` test, and a full icacls/NTFS
    // ACL parse is brittle and out of scope. So the gap is made EXPLICIT rather
    // than silent: by default we emit a one-line informational warning that
    // perm checks are unavailable on this platform, and when the operator opts
    // into strict mode via `TENSOR_WASM_REQUIRE_KEY_PERMS=1` we REFUSE (fail
    // closed) — strict mode asked us to verify the key file is locked down and
    // we cannot make that guarantee on Windows, so we must not silently proceed.
    // The operator should secure the key file via NTFS ACLs (e.g. `icacls`
    // restricting access to the owning service account).
    #[cfg(unix)]
    {
    ...
    #[cfg(windows)]
    {
        // L-8: no POSIX-style permission bits on Windows. Make the missing
        // check explicit instead of silently skipping it.
        let strict = std::env::var("TENSOR_WASM_REQUIRE_KEY_PERMS")
            .map(|v| v == "1")
            .unwrap_or(false);
        if strict {
            // Fail CLOSED: strict mode demands a verified-locked-down key file,
            // and we cannot verify NTFS ACLs here, so refuse rather than give a
            // false assurance.
            return Err(local_err(format!(
                "HMAC key file {} cannot be permission-checked on Windows and \
                 TENSOR_WASM_REQUIRE_KEY_PERMS=1 is set, so refusing to use it — \
                 file-permission verification is not implemented on this platform; \
                 secure the key file via NTFS ACLs (e.g. `icacls` restricting access \
                 to the owning account) and unset the variable to proceed",
                path.display()
            )));
        }
        tracing::warn!(
            target: "tensor_wasm_cli::snapshot",
            file = %path.display(),
            "HMAC key file permission checks are not available on Windows; \
             ensure the key file is secured via NTFS ACLs (the file holds a \
             32-byte signing secret and any reader can forge snapshots) — set \
             TENSOR_WASM_REQUIRE_KEY_PERMS=1 to refuse when the check is \
             unavailable"
        );
    }
