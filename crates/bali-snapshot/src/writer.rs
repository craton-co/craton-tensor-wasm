...
//! container. The on-disk magic value `0xBA11_5HOT` and a `version` field let
...
/// Magic bytes that identify a Bali snapshot blob.
///
...
/// `BA11 5407` ("BALI SHOT") in leet hex.
...
/// deliberately generous — large enough to fit any plausible workload Bali
...
/// vectors are opaque to this crate — upper layers (`bali-mem`, `bali-exec`)
/// own the schema of what they put in.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize)]
pub struct Snapshot {
    /// Magic identifying this blob as a Bali snapshot. Must equal [`SNAPSHOT_MAGIC`].
    pub magic: u32,
...
/// just to call into this crate — the writer takes ownership only of the
/// metadata fields and copies the byte slices once into the [`Snapshot`] body.
...
pub(crate) fn check_blob_size(kind: &'static str, len: usize, max: usize) -> Result<()> {
    if len > max {
        Err(BaliError::Serialization(format!(
            "snapshot {kind} too large: {len} bytes exceeds cap of {max} bytes",
        )))
    } else {
        Ok(())
    }
...
impl SnapshotWriter {
    ...
    /// `Vec<u8>` as-is. Returns [`BaliError::Serialization`] if bincode encoding
    ...
    pub fn capture(&self, state: InstanceState<'_>) -> Result<Vec<u8>> {
        ...
        let encoded = bincode::serialize(&snapshot)
            .map_err(|e| BaliError::Serialization(format!("bincode encode: {e}")))?;
        let compressed = zstd::encode_all(encoded.as_slice(), self.zstd_level)
            .map_err(|e| BaliError::Serialization(format!("zstd encode: {e}")))?;
