//! Hardening tests for `bali-snapshot` capture and restore.
//!
...
fn restore_rejects_oversized_registers_via_handcrafted_blob() {
    ...
    let crc32 = bali_snapshot::payload_crc32(&[], &[], &registers);
    let snap = Snapshot {
...
fn restore_rejects_magic_mismatch() {
    ...
    let snap = Snapshot {
        ...
        crc32: bali_snapshot::payload_crc32(&[], &[], &[]),
    };
    let encoded = bincode::serialize(&snap).expect("bincode");
    let compressed = zstd::encode_all(encoded.as_slice(), DEFAULT_ZSTD_LEVEL).expect("zstd");
    let err = reader
