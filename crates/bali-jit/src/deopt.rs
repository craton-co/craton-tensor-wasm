...
pub struct DeoptGuard {
    ...
    metrics: Option<bali_core::metrics::BaliMetrics>,
}
...
impl DeoptGuard {
    ...
    /// to the supplied [`bali_core::metrics::BaliMetrics`] handle. The handle is
    /// `Arc`-backed and cheap to clone — pass the same handle used by the rest
    /// of the process so the `bali_offload_success_total` and
    /// `bali_offload_fallback_total` counters reflect the kernels driven by
    /// this guard.
    pub fn with_metrics(metrics: bali_core::metrics::BaliMetrics) -> Self {
        Self {
            deopted: DashMap::new(),
            success_total: AtomicU64::new(0),
            fallback_total: AtomicU64::new(0),
    ...
    /// are wired, the `bali_offload_fallback_total` Prometheus counter).
    ...
    /// if metrics are wired, the `bali_offload_success_total` Prometheus
    /// counter).
...
mod tests {
    ...
    fn with_metrics_increments_offload_success_total() {
        ...
        // Bali metrics counter
        let text = metrics.encode_text();
        assert!(
            text.contains("bali_offload_success_total 3"),
            "got:\n{text}"
        );
    }
    ...
    fn with_metrics_increments_offload_fallback_total() {
        ...
        let text = metrics.encode_text();
        assert!(
            text.contains("bali_offload_fallback_total 2"),
            "got:\n{text}"
        );
    }
