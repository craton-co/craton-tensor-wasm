...
//! [`bali_jit::detector::classify`]. Verdicts are emitted as `tracing`
...
//! upstream hooks Bali v0.1.0 does not have.

use bali_jit::detector::{classify_default, BlockIR, DetectorVerdict, Op};
    ...
    for payload in Parser::new(0).parse_all(wasm) {
        ...
        if let Payload::CodeSectionEntry(body) = payload {
            ...
            let block = BlockIR::new(
                ...
                detector_ops.clone(),
                // Without a static trip-count analysis here we conservatively
                // claim Some(128) — a reasonable hot-loop default. The detector
                // will only flag if the v128 ratio also clears the threshold.
                Some(128),
            );
