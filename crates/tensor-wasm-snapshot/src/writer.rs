...
use crate::format::{
    SIGNATURE_KIND_ED25519, SIGNATURE_KIND_HMAC_SHA256, SNAPSHOT_VERSION_V3, V3_TRAILER_MAGIC,
};
#[cfg(feature = "signed-snapshots")]
...
/// bump.
///
/// **Replay protection (LANDED).** The trailing
/// [`SnapshotMetadata::sequence_no`] and [`SnapshotMetadata::nonce`] fields
/// are now driven end-to-end: the writer stamps them via
/// [`SnapshotWriter::with_sequence_no`] / [`SnapshotWriter::with_nonce`] and
/// the reader enforces them via
/// [`crate::reader::SnapshotReader::with_min_sequence_no`] /
/// [`crate::reader::SnapshotReader::with_expected_nonce`]. Both default to the
/// v0.3.x "unset" values (`0` / `None`) so a writer that does not opt in
/// stays semantically backward-compatible. Callers that rely on bincode-exact
/// compatibility with pre-field v0.3.x blobs must regenerate captures after
/// upgrading — the on-disk byte layout carries 9 extra bytes per metadata
/// record (a `u64` plus the `Option<[u8;16]>` discriminant; +16 more when the
/// nonce is `Some`).
...
pub struct SnapshotMetadata {
    ...
    /// Monotonic counter stamped by the writer via
    /// [`SnapshotWriter::with_sequence_no`]. Operators that want strict
    /// replay/rollback protection track the highest seen `sequence_no` per
    /// signing key and construct the reader with
    /// [`crate::reader::SnapshotReader::with_min_sequence_no`] so any blob
    /// with `sequence_no` below the floor is refused.
    ///
    /// Defaults to `0` for writers that have not called
    /// [`SnapshotWriter::with_sequence_no`] — the v0.3.x "unset" value. For
    /// v3 blobs the signature covers this field (it lives inside the signed
    /// bincode payload), so a replayed capture cannot rewrite it.
    #[cfg_attr(feature = "signed-snapshots", serde(default))]
    pub sequence_no: u64,
    /// Optional caller-supplied nonce stamped via
    /// [`SnapshotWriter::with_nonce`]. When the reader is configured with
    /// [`crate::reader::SnapshotReader::with_expected_nonce`], a blob is
    /// accepted only if its nonce matches exactly. Defaults to `None` —
    /// backward compatible with writers that have not opted in.
    ///
    /// For v3 blobs the signature covers this field, so a replayed capture
    /// cannot substitute a matching nonce without re-signing.
    #[cfg_attr(feature = "signed-snapshots", serde(default))]
...
pub struct SnapshotWriter {
    ...
    /// Ed25519 *private* signing key used to sign v3 snapshots with the
    /// asymmetric `signature_kind = 2` trailer. `None` -> no Ed25519
    /// signature is emitted.
    ///
    /// Mutually exclusive with [`SnapshotWriter::hmac_key`] at capture time:
    /// if both are configured the Ed25519 key wins (the asymmetric trailer
    /// is emitted) so a writer cannot accidentally double-sign. The field is
    /// `pub(crate)` rather than `pub` so callers cannot read the private key
    /// back out by name once configured.
    ///
    /// `ed25519_dalek::SigningKey` zeroizes its own secret scalar on drop
    /// (the dalek crate implements `Drop`/`ZeroizeOnDrop` for it), so no
    /// extra `Zeroizing` wrapper is needed here.
    #[cfg(feature = "signed-snapshots")]
    pub(crate) ed25519_key: Option<SigningKey>,
    /// Replay-protection sequence number written into
    /// [`SnapshotMetadata::sequence_no`]. Set via
    /// [`SnapshotWriter::with_sequence_no`]; defaults to `0` (the v0.3.x
    /// "unset" value). Operators that want rollback defence assign a
    /// per-key monotonic counter here.
    pub(crate) sequence_no: u64,
    /// Replay-protection nonce written into [`SnapshotMetadata::nonce`].
    /// Set via [`SnapshotWriter::with_nonce`]; defaults to `None`
    /// (backward-compatible with operators that have not opted into the
    /// nonce check).
    ...
    /// Deterministic override for [`SnapshotMetadata::created_unix_ms`].
    ///
    /// `None` (the default) stamps `SystemTime::now()` at capture time. When
    /// set via [`SnapshotWriter::with_created_unix_ms`], that exact value is
    /// written instead, making captures bit-reproducible — useful for
    /// reproducible-build verification (see `docs/REPRODUCIBLE-BUILDS.md`) and
    /// for deterministic tests that compare two captures of the same input.
    pub(crate) fixed_created_unix_ms: Option<u64>,
}
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        d.field(
            ...
            &self
                .ed25519_key
                .as_ref()
                .map(|_| "<REDACTED Ed25519 signing key>"),
        );
        d.field("sequence_no", &self.sequence_no);
        d.field("nonce", &self.nonce);
        d.field("use_legacy_envelope", &self.use_legacy_envelope);
        d.field("fixed_created_unix_ms", &self.fixed_created_unix_ms);
        d.finish()
    }
}
...
impl SnapshotWriter {
    ...
    /// Pin [`SnapshotMetadata::created_unix_ms`] to a fixed value instead of
    /// stamping `SystemTime::now()` at capture time.
    ///
    /// With this set, two captures of the same input produce **byte-identical**
    /// blobs. The timestamp is the only non-deterministic field, and it flows
    /// through the bincode+zstd stream — so even though it is a fixed-width
    /// `u64`, changing its *value* can shift the compressed length by a byte.
    /// Pinning it makes captures reproducible (see `docs/REPRODUCIBLE-BUILDS.md`)
    /// and lets tests assert determinism without freezing the system clock.
    /// Production callers should leave it unset so each snapshot records its
    /// real wall-clock time.
    #[must_use]
    pub const fn with_created_unix_ms(mut self, created_unix_ms: u64) -> Self {
        self.fixed_created_unix_ms = Some(created_unix_ms);
        self
    }
    ...
    /// Configure Ed25519 *asymmetric* signing with a private signing key.
    ///
    /// Once configured, [`SnapshotWriter::capture`] emits a v3 envelope
    /// whose trailer is `[V3_TRAILER_MAGIC: 4][signature_kind = 2: 1]
    /// [Ed25519 signature: 64]` (69 bytes total). The signature covers the
    /// same byte range the HMAC path covers — the entire v2-shaped
    /// compressed prefix concatenated with the trailer magic and the
    /// signature-kind byte — so a blob cannot be downgraded from Ed25519 to
    /// HMAC (or vice-versa) by rewriting the kind byte without invalidating
    /// the signature. See `FORMAT.md` for the exact byte layout.
    ///
    /// Unlike [`SnapshotWriter::with_hmac_sha256_key`], the verifier side
    /// only needs the *public* key
    /// ([`ed25519_dalek::VerifyingKey`], obtained via
    /// `signing_key.verifying_key()`): a publisher can sign with a private
    /// key held in one place while arbitrarily many verifiers hold only the
    /// public key and cannot forge new snapshots. This is the asymmetric
    /// counterpart to the symmetric HMAC path — choose it when the verifiers
    /// are not in the same trust domain as the signer.
    ///
    /// If both an Ed25519 key and an HMAC key are configured on the same
    /// writer, the Ed25519 key takes precedence at capture time (the
    /// asymmetric trailer is emitted) — a writer never double-signs.
    ///
    /// The key is held by value and never logged or surfaced in error
    /// messages; `ed25519_dalek::SigningKey` scrubs its secret scalar on
    /// drop. Treat the writer as secret once this method has been called.
    #[cfg(feature = "signed-snapshots")]
    #[cfg_attr(docsrs, doc(cfg(feature = "signed-snapshots")))]
    #[must_use]
    pub fn with_ed25519_signing_key(mut self, key: SigningKey) -> Self {
        self.ed25519_key = Some(key);
        self
    }

    /// Set the monotonic [`SnapshotMetadata::sequence_no`] written into the
    /// snapshot, for replay/rollback defence.
    ///
    /// The writer no longer hard-codes `0`: operators that want strict
    /// replay protection assign a per-signing-key monotonic counter here
    /// (e.g. the current value of a persisted `last_written` counter for the
    /// key, incremented on each capture). The reader-side floor is
    /// [`crate::reader::SnapshotReader::with_min_sequence_no`].
    ///
    /// Leaving this unset keeps the v0.3.x default of `0`.
    #[must_use]
    pub const fn with_sequence_no(mut self, sequence_no: u64) -> Self {
        self.sequence_no = sequence_no;
        self
    }

    /// Set the [`SnapshotMetadata::nonce`] written into the snapshot, for
    /// replay defence keyed on a caller-chosen value.
    ///
    /// When set, a reader configured with
    /// [`crate::reader::SnapshotReader::with_expected_nonce`] accepts the
    /// blob only if the nonces match — letting a restorer pin a snapshot to
    /// a single expected challenge value and reject any other (including a
    /// replayed older capture that carried a different nonce).
    ///
    /// Leaving this unset keeps the v0.3.x default of `None`.
    #[must_use]
    ...
    fn build_metadata(&self, state: &InstanceState<'_>) -> Result<(SnapshotMetadata, u32)> {
        ...
        // can proceed.
        //
        // A deterministic override (`with_created_unix_ms`) short-circuits the
        // clock read entirely, so reproducible-build callers and tests can pin
        // the timestamp and get byte-identical captures.
        let created_unix_ms = match self.fixed_created_unix_ms {
            Some(ms) => ms,
            None => SystemTime::now()
                ...
                .map_err(|e| {
                    TensorWasmError::Serialization(
                        format!(
                            "snapshot created_unix_ms: SystemTime::now() before UNIX_EPOCH: {e}"
                        )
                        .into(),
                    )
                })
                .and_then(|d| {
                    u64::try_from(d.as_millis()).map_err(|_| {
                        TensorWasmError::Serialization(
                            "snapshot created_unix_ms: milliseconds since epoch overflows u64"
                                .into(),
                        )
                    })
                })?,
        ...
        let metadata = SnapshotMetadata {
            ...
            // Replay-protection fields are now writer-driven. They default
            // to `0` / `None` (the v0.3.x "unset" semantics) unless the
            // caller opted in via `SnapshotWriter::with_sequence_no` /
            // `SnapshotWriter::with_nonce`. The reader enforces them via
            // `with_min_sequence_no` / `with_expected_nonce`; a writer that
            // leaves them unset stays byte-compatible with pre-opt-in
            // captures (modulo the +9-byte metadata growth already on the
            // wire).
    ...
    /// ```
    ///
    /// In every other configuration — feature off, no HMAC key, the
    /// legacy-envelope opt-out set, **or an Ed25519 signing key configured**
    /// — the writer emits the legacy v2/v3 inline envelope
    /// (`zstd(bincode(Snapshot)) [|| v3 trailer]`). An Ed25519 key forces
    /// the inline path because the artifact-store envelope authenticates
    /// with HMAC only and has no slot for an asymmetric signature.
    ...
    pub fn capture(&self, state: InstanceState<'_>) -> Result<Vec<u8>> {
        ...
        // An Ed25519 key forces the legacy inline envelope: the artifact-store
        // envelope authenticates with HMAC-SHA256 only, so there is nowhere to
        // carry an asymmetric signature on that path. Routing Ed25519 captures
        // through `capture_legacy` keeps the asymmetric trailer the single
        // source of truth and avoids silently dropping it.
        #[cfg(all(feature = "artifact-backing", feature = "signed-snapshots"))]
        {
            if !self.use_legacy_envelope && self.ed25519_key.is_none() {
    ...
    pub fn capture_legacy(&self, state: InstanceState<'_>) -> Result<Vec<u8>> {
        ...
        // Pick the on-wire version. Without any signing key configured we emit
        // the legacy unsigned v2 envelope; with either an Ed25519 or HMAC key
        // we bump to v3 so the reader (which keys signature handling off the
        // version field plus the trailer kind byte) knows to look for the
        // trailing signature kind + signature.
        #[cfg(feature = "signed-snapshots")]
        let on_wire_version = if self.ed25519_key.is_some() || self.hmac_key.is_some() {
            SNAPSHOT_VERSION_V3
        } else {
            SNAPSHOT_VERSION
        };
        ...
        //
        // Ed25519 (snapshot 1.2) takes precedence over HMAC: the asymmetric
        // trailer is `[magic][kind = 2][64-byte signature]` (69 bytes). The
        // signed message is byte-identical in *shape* to the HMAC input —
        // `compressed_prefix || V3_TRAILER_MAGIC || [kind_byte]` — so the
        // kind byte is authenticated and an Ed25519 blob cannot be presented
        // as HMAC (or vice-versa) by rewriting the kind byte.
        #[cfg(feature = "signed-snapshots")]
        if let Some(ref signing_key) = self.ed25519_key {
            // Sign `compressed || V3_TRAILER_MAGIC || [SIGNATURE_KIND_ED25519]`.
            // ed25519-dalek's `Signer::sign` is infallible for the in-memory
            // key path (it returns a `Signature`, not a `Result`). We build
            // the message once into a small scratch buffer — the prefix is
            // already in `compressed`, so we only append the 5 trailer-header
            // bytes before signing, then push the signature.
            let mut message = Vec::with_capacity(compressed.len() + V3_TRAILER_MAGIC.len() + 1);
            message.extend_from_slice(&compressed);
            message.extend_from_slice(&V3_TRAILER_MAGIC);
            message.push(SIGNATURE_KIND_ED25519);
            let sig = signing_key.sign(&message);
            let sig_bytes = sig.to_bytes();
            compressed.reserve(V3_TRAILER_MAGIC.len() + 1 + sig_bytes.len());
            compressed.extend_from_slice(&V3_TRAILER_MAGIC);
            compressed.push(SIGNATURE_KIND_ED25519);
            compressed.extend_from_slice(&sig_bytes);
        } else if let Some(ref key) = self.hmac_key {
            use hmac::{Hmac, Mac};
            use sha2::Sha256;
...
mod tests {
    ...
    /// `with_sequence_no` / `with_nonce` populate the metadata fields that
    /// were previously hard-coded to `0` / `None`.
    #[test]
    fn replay_fields_are_written_into_metadata() {
        let (wasm, gpu, regs) = sample_state();
        let nonce = [0x5Au8; 16];
        let bytes = SnapshotWriter::new()
            .with_sequence_no(99)
            ...
            .expect("capture");
        let restored = SnapshotReader::new().restore(&bytes).expect("restore");
        assert_eq!(restored.metadata.sequence_no, 99);
        assert_eq!(restored.metadata.nonce, Some(nonce));
    }

    /// Default writer still emits the v0.3.x "unset" replay values.
    #[test]
    fn replay_fields_default_to_unset() {
        let (wasm, gpu, regs) = sample_state();
        let bytes = SnapshotWriter::new()
            .capture(InstanceState {
                tenant_id: TenantId(1),
        ...
        assert_eq!(restored.metadata.nonce, None);
    }

    /// An Ed25519-configured writer emits a v3 blob with the 69-byte
    /// asymmetric trailer (`[magic: 4][kind = 2: 1][sig: 64]`) and it
    /// round-trips through a reader holding only the *public* key.
    #[cfg(feature = "signed-snapshots")]
    #[test]
    fn ed25519_writer_emits_v3_and_round_trips() {
        ...
        // Trailer is `[magic: 4][kind = 2: 1][sig: 64]` = 69 bytes.
        assert!(bytes.len() >= crate::format::ED25519_TRAILER_LEN);
        let trailer_start = bytes.len() - crate::format::ED25519_TRAILER_LEN;
        assert_eq!(
            &bytes[trailer_start..trailer_start + crate::format::V3_TRAILER_MAGIC_LEN],
            &crate::format::V3_TRAILER_MAGIC,
        );
        assert_eq!(
            bytes[trailer_start + crate::format::V3_TRAILER_MAGIC_LEN],
            crate::format::SIGNATURE_KIND_ED25519,
        );

        let restored = SnapshotReader::new()
            .with_ed25519_verifying_key(verifying_key)
            .restore(&bytes)
            .expect("restore ed25519 v3");
        assert_eq!(restored.wasm_memory, wasm);
        assert_eq!(restored.gpu_memory, gpu);
        assert_eq!(restored.registers, regs);
        assert_eq!(restored.version, crate::format::SNAPSHOT_VERSION_V3);
