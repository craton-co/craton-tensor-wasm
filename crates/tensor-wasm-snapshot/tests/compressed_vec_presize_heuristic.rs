...
fn pre_size_does_not_change_wire_bytes() {
    // Property under test: the heuristic only affects allocator behaviour,
    // not the produced byte stream. Two captures of the same input must
    // produce the same length of compressed payload (modulo the metadata
    // timestamp, which is non-deterministic). We can't easily compare
    // byte-for-byte without freezing the clock, but the bincode-encoded
    // `created_unix_ms` field is a fixed-width `u64`, so the total blob
    // length is independent of the timestamp value. If the pre-size
    // somehow leaked into the payload (e.g. a stray write into the
    // `Vec`'s spare capacity), the two lengths would diverge.
    ...
    assert_eq!(
        a.len(),
        b.len(),
        "two captures of the same input must produce the same blob length \
         — divergence would imply the pre-size heuristic leaked into the \
         compressed payload",
    );
}
