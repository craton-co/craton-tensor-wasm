...
- **Kernel-PTX confidentiality between tenants.** The kernel registry is a
  deployment-global, operator-curated namespace: publishing is gated, but
  reading is not tenant-scoped. Any authenticated tenant can list and
  resolve (including full PTX source) any kernel any other tenant published.
  This is an explicit, signed-off design decision, acceptable only when all
  tenants in a deployment are mutually trusted to read each other's kernel
  PTX; deployments with mutually-untrusting tenants must not rely on the
  registry for confidential kernels. See the "Kernel registry shared-read
  model" entry in [`docs/RISKS.md`](docs/RISKS.md) for the rationale, trust
  boundary, and the manifest tenant/visibility dimension proposed as future
  work. Note this is a *confidentiality* scoping decision only — it does not
  weaken tenant memory or execution isolation (the registry hands back PTX
  text, not another tenant's memory or launches).
...
during the preview window; the latest patch is `0.3.7` (released
2026-05-28). The `0.1.x` line is also supported during the
