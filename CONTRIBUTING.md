...
`RUSTFLAGS=-D warnings` and CUDA stub libs, exactly as CI does. By default it
runs **two containers in parallel** — a `tests` lane (the `test` job) and a
`checks` lane (fmt, clippy, doc, deny, cuda-oxide, openapi, actionlint) — each
on its own target-cache volume, so the long test pass overlaps the lint/doc/
audit battery. Pass `--serial` (`-Serial` in PowerShell) for the old
single-container behaviour with one shared warm cache. The Linux build cache
lives on Docker volumes, so it never touches your host `target/`. The macOS
compile-test job is not reproduced (it needs a macOS host).
