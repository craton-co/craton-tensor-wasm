...
#
#   docker build --build-arg BACKEND=cust       -t tensor-wasm:0.3.7-cust       .
#   docker build --build-arg BACKEND=cudarc     -t tensor-wasm:0.3.7-cudarc     .
#   docker build --build-arg BACKEND=cuda-oxide -t tensor-wasm:0.3.7-cuda-oxide .
#   docker build                                -t tensor-wasm:0.3.7            .  # host-only (no CUDA)
#
...
#
# Registry status as of v0.3.7: the ghcr.io path is NOT yet
...
# Stage 1 — builder
# ---------------------------------------------------------------------------
FROM rust:1.84-slim-bookworm@sha256:f1c6e953d9cfe4bd8eb4512a82647ef68965484714eb63d925b955458a357133 AS builder

ARG BACKEND
# Re-declare inside the stage so it's available in RUN.
ARG WORKSPACE_VERSION=0.3.7
